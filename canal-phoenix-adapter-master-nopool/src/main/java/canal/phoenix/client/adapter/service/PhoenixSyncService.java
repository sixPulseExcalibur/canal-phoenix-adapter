...
import canal.phoenix.client.adapter.support.BatchExecutor;
import canal.phoenix.client.adapter.support.SingleDml;
...
public class PhoenixSyncService {
    ...
    //同步线程数
    //private int   threads = 3;   //默认开启3个线程同步,此处配置了自定义同步线程数
    ...
    //相对于RdbSyncService方法少了参数 boolean skipDupException
    public PhoenixSyncService(DataSource dataSource, Integer threads) {
    ...
    //相遇于RdbSyncService方法少了参数 boolean skipDupException
    @SuppressWarnings("unchecked")
    private PhoenixSyncService(DataSource dataSource, Integer threads, Map<String, Map<String, Integer>> columnsTypeCache) {
    ...
    private void insert(BatchExecutor batchExecutor, MappingConfig config, SingleDml dml) throws SQLException {
        ...
        //【CHANGE】phoenix需要将连接池获取连接改为直接获取连接
    ...
    private void appendCondition(DbMapping dbMapping, StringBuilder sql, Map<String, Integer> ctype,
        ...
        for (Map.Entry<String, String> entry : dbMapping.getTargetPk().entrySet()) {
            ...
            //logger.info("d.get(srcColumnName)的值:"+d.get(srcColumnName));
            //logger.info("getHashRowKeys(String.valueOf(d.get(srcColumnName))的值: "+getHashRowKeys(String.valueOf(d.get(srcColumnName))));
     ...
     * hbase表分区数
     */
    public static final int REGION_COUNT= ConfigurationManager.getInteger("regionCount");
     ...
     * @return
     */
    public static String getHashRowKeys(String rowKeyValue){
        //可使用主键反转
        //StringBuilder rowkey= new StringBuilder(rowKey).reverse();
        // 分区号
        int regionNum;
        //rowKeyValue的hash值
        int hash=rowKeyValue.hashCode();
        //加载配置文件hbase_common.properties获取分区数
        int regionCount=REGION_COUNT;
        if(regionCount>0 && (regionCount & (regionCount-1)) == 0){
            // 如果分区数是2^n整数倍,则使用位运算确定rowkey的分区(因为这种情况下,余运算可以转化为位运算,而且位运算的速度远远高于余运算)
            regionNum=hash & (regionCount-1);
        }else{
            // 否则使用余运算确定rowkey的分区
            regionNum=hash % regionCount;
        }
        String hashRowKeys=Math.abs(regionNum)+"_"+rowKeyValue.toString();
        return hashRowKeys;
    }
    ...
    private int pkHash(DbMapping dbMapping, Map<String, Object> d) {
        ...
        for (Map.Entry<String, String> entry : dbMapping.getTargetPk().entrySet()) {
            ...
            if (value != null) {
                hash += value.hashCode();
                //logger.info("hash前的值"+value);
            }
        }

        hash = Math.abs(hash) % threads;
        //logger.info("hash后的值"+Math.abs(hash));
